...
as $n \rightarrow \infty$ finishing the upper bound proof.
...
Let $A_1, ... , A_m$ be subsets of [N] and $Y$ be a set in $[N]$ whose elements are chosen independently, with $\mathbb{P}(j \in Y) = p$ for each $j$. 
...
$$ \Delta = \sum_{i\sim j} \mathbb{P}(B_i \wedge B_j), $$
...
& = \mathbb{P}(B_j) - \sum_{i \sim j, i < j} \mathbb{P}(B_i | B_j).
\end{align*}
...
Then we finish the proof of the first inequality.
\end{proof}
