...
$$R(k,\ell) \leq \binom{k+\ell}{k}.$$
So,
$$R(k-1,\ell)\leq \binom{k-1+\ell}{k-1}.$$ and $$R(k,\ell-1)\leq \binom{k+\ell-1}{\ell-1}.$$
That implies on:
$$R(k,\ell)\leq \binom{k-1+\ell}{k-1} + \binom{k+\ell-1}{\ell-1} = \binom{k+ \ell}{k}.$$
...
it follows if $n<(1/e\sqrt{2})k2^{k/2}$. Once the expected value for $X$ is less than $1$, then there must exist a colouring in which $X=0.$
\end{proof}
...
Turán generalized its result in 1941. It uses $T_r(n)$ which is a complete $r$-partite graph with $n$ vertices and $\lceil \frac{n}{r} \rceil$ or $\lfloor \frac{n}{r} \rfloor$ vertices on each part.

\begin{theorem} \label{theorem:turan1941}(Turán, 1941). $ex(n, k_{r+1}) = t_r(n)$.
\end{theorem}
\begin{proof}
This is a proof by induction on $n$. Let $G$ be maximum without a $K_{r+1}$. Construct a graph $G'$ removing a $K_r$ from $G$, so:
\begin{align*}
