...
Choose $n \geq R(k-1,l) + R(k, l-1)$ and pick any vertex $v$ from $[n]$. Using Pigeonhole principle we realize $v$ has either at least $R(k-1,l)$ red neighbours, or at least $R(k,l-1)$ blue neighbours, for simplicity assume that $v$ has at least $R(k-1,l)$ red neighbors. By definition we have on the red neighbors of $v$ a red $K_{k-1}$ or a blue $K_l$, if it is a $K_l$ we are done and if it is a $K_{k-1}$ we can add $v$, forming a $K_k$. We just proved we always have a $K_k$ red or a $K_l$ blue with such $n$ and it is a upper bound for equation $R(k,l) \leq R(k-1,l)+R(k,l-1)$ because it doesn't prove this $n$ is the minimum one, only shows that for this specific $n$ it is true.\\
...
\begin{align*}
e(G-\{u,v\}) &\leq (\lfloor \frac{n}{2} \rfloor -1)(\lceil \frac{n}{2} \rceil -1) \\
&= \lfloor \frac{n}{2} \rfloor \lceil \frac{n}{2} \rceil - n +1
\end{align*}
...
Now we use the fact $\left(\frac{n}{m}\right)^m \leq \binom{n}{m} \leq \left(\frac{en}{m}\right)^m$ and Jensen's Inequality \ref{prep:jensen} to acquire the following,
...
\end{align*}
$c(s) = \left(\frac{2}{s}\right)^s$ and it is only a simplification for this constant which depends of $s$ value.
...
\end{align*}
Using again the fact $\binom{n}{m} \leq \left(\frac{en}{m}\right)^m$ and the constant simplification for $c(s,t)$ we have what we wanted to show  and the proof is finished.
\end{proof}
...
$$\mathbb{P}\binom{\chi{X}(G(n,p)) \geq k \text{ and }} {g(G(n,p)) \geq k} > 0 ,$$
...
Being $S$ a independent set in $G(n,p)$ and $|S| = \frac{n}{k}$,
...
$$\mathbb{E}[X_k] = \sum_{C_k \in K_n} \mathbb{P}(C_k) \leq n^kp^k,$$
...
And we can remove a edge of each cycle with length $<k$, so we have $g(G) \geq k$. Note removing a cycle's edge doesn't make $\alpha(G)$ bigger and consequently doesn't make $\chi(G)$ smaller, maintaining all our previous calculations valid.
\end{proof}
