...
Then we have at least $ (\delta - \epsilon)^3 n^2$ triangles for each $v$ then follows we have at least $(1-2\epsilon) (\delta-\epsilon)^3 n^3$ triangles in $G$. 
...
\begin{theorem}
(The Szemerédi Regularity Lemma, 1975). Let $\epsilon > 0$, and let $m \in  \mathbb{N}$. There exists a constant $M=M(m, \epsilon)$ such that the following holds.\\
For any graph $G$, there exists a partition $V(G) = A_0 \cup ... \cup A_k$ of the vertex set into $m\leq k \leq M$ parts, such that

$|A_1| = ... =|A_k|$,

$|A_0| \leq \epsilon |V(G)|$,

all but $\epsilon k^2$ of the pairs $(A_i, A_j)$ are $\epsilon$-regular. 
\end{theorem}
