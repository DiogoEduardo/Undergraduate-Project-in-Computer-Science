...
\begin{proof}
Here we only give the proof for $H=K_3$. Let %$G \in \mathcal{G}(H, m,\epsilon,\delta)$ and let
...
Note the lemma holds for all graphs, but it is useful only for graphs with large $n$ and with a minimum density.% Indeed, if $|V(G)| < M(m, \epsilon)$ then each part has at most one vertex and if $e(G) = o(n^2)$ then $G$ is well-approximated by the empty graph, in both the lemma is vacuous and not so useful.  
